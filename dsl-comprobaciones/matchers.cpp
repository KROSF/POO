...
DeclarationMatcher dynamic_cast_Matcher(string methodName, string className) {
  DeclarationMatcher matcher =
      cxxMethodDecl(
          anyOf(matchesName(methodName), hasName(methodName)),
          unless(isImplicitMethod()),
          forEachDescendant(
              cxxDynamicCastExpr(
                  hasDescendant(implicitCastExpr().bind("dynamic_type")))
                  .bind("dynamic_method")),
          ofClass(cxxRecordDecl(matchesName(className),
...
DeclarationMatcher defaultConstructor_Matcher(string className) {
  DeclarationMatcher matcher =
      cxxConstructorDecl(
          isDefaultConstructor(), unless(isImplicit()),
          ofClass(cxxRecordDecl(matchesName(className),
                                unless(isTemplateInstantiation()))
...
DeclarationMatcher copyConstructor_Matcher(string className) {
  DeclarationMatcher matcher =
      cxxConstructorDecl(
          isCopyConstructor(), unless(isDefaulted()), unless(isImplicit()),
          ofClass(cxxRecordDecl(matchesName(className),
                                unless(isTemplateInstantiation()))
...
DeclarationMatcher moveConstructor_Matcher(string className) {
  DeclarationMatcher matcher =
      cxxConstructorDecl(
          isMoveConstructor(), unless(isDefaulted()), unless(isImplicit()),
          ofClass(cxxRecordDecl(matchesName(className),
                                unless(isTemplateInstantiation()))
...
DeclarationMatcher destructor_Matcher(string className) {
  DeclarationMatcher matcher =
      cxxDestructorDecl(unless(isImplicitDestructor()),
                        ofClass(cxxRecordDecl(matchesName(className),
                                              unless(isTemplateInstantiation()))
...
DeclarationMatcher destructorDeleteMember_Matcher(string className,
                                                  string memberName) {
  DeclarationMatcher matcher = cxxDestructorDecl(
      hasDescendant(
          cxxDeleteExpr(
              hasDescendant(memberExpr(hasDeclaration(
                  fieldDecl(matchesName(memberName)).bind("deleteMember")))))
...
DeclarationMatcher takeException_Matcher(string exceptionClass) {
  DeclarationMatcher matcher = functionDecl(
      ...
      hasDescendant(cxxThrowExpr(forEachDescendant(cxxConstructExpr(
                                     hasDeclaration(cxxMethodDecl(ofClass(
                                         cxxRecordDecl(hasName(exceptionClass))
                                             .bind("exceptionClass")))))))
                        .bind("exception")));
  return matcher;
}
...
DeclarationMatcher member_Matcher(string className, string memberName) {
  DeclarationMatcher matcher =
      cxxRecordDecl(has(fieldDecl(matchesName(memberName)).bind("foundMember")),
                    matchesName(className), unless(isTemplateInstantiation()))
          .bind("foundClass");
  return matcher;
}
...
DeclarationMatcher variableDelete_Matcher() {
  DeclarationMatcher matcher = functionDecl(
      unless(isImplicitFunction()),
      forEachDescendant(cxxDeleteExpr(
          hasDescendant(declRefExpr(to(varDecl().bind("variable_delete")))))));
  return matcher;
}

DeclarationMatcher allPrivateMember_Matcher(string className) {
  DeclarationMatcher matcher =
      cxxRecordDecl(has(fieldDecl(isPublic())), matchesName(className),
                    unless(isTemplateInstantiation()))
...
DeclarationMatcher memberVariableAccessLevel_Matcher(string className,
  ...
  DeclarationMatcher matcher =
      cxxRecordDecl(
          forEachDescendant(fieldDecl(matchesName(memberName))
...
DeclarationMatcher notFriendMember_Matcher(string className) {
  DeclarationMatcher matcher =
      cxxRecordDecl(matchesName(className), hasDescendant(friendDecl()),
                    unless(isTemplateInstantiation()))
...
DeclarationMatcher friendFunction_Matcher(string functionName,
  ...
  DeclarationMatcher matcher =
      cxxRecordDecl(
          hasDescendant(friendDecl(has(
              functionDecl(matchesName(functionName)).bind("friendFunction")))),
          matchesName(className), unless(isTemplateInstantiation()))
...
DeclarationMatcher friendClass_Matcher(string className,
  ...
  DeclarationMatcher matcher =
      cxxRecordDecl(hasDescendant(friendDecl(
...
DeclarationMatcher method_Matcher(string methodName, string className) {
  DeclarationMatcher matcher =
      cxxMethodDecl(anyOf(matchesName(methodName), hasName(methodName)),
                    unless(isImplicitMethod()),
                    ofClass(cxxRecordDecl(matchesName(className),
                                          unless(isTemplateInstantiation()))
...
DeclarationMatcher methodWithReferencedVariable_Matcher(string methodName,
                                                        ...
                                                        string usedVariable) {
  DeclarationMatcher matcher =
      cxxMethodDecl(forEachDescendant(memberExpr(
                        hasDeclaration(fieldDecl(matchesName(usedVariable))
                                           .bind("referencedVariable")))),
                    anyOf(matchesName(methodName), hasName(methodName)),
                    ofClass(cxxRecordDecl(matchesName(className),
                                          unless(isTemplateInstantiation()))
...
DeclarationMatcher functionWithReferencedMethod_Matcher(string mainFunctionName,
  ...
  DeclarationMatcher matcher =
      functionDecl(
          ...
          anyOf(hasDescendant(memberExpr(hasDeclaration(
                    cxxMethodDecl(
                        anyOf(matchesName(usedMethodName),
                              hasName(usedMethodName)),
                        ofClass(cxxRecordDecl(matchesName(usedClassName),
                                              unless(isTemplateInstantiation()))
                                    .bind("classOfReferencedMethodByFunction")))
                        .bind("usedMethodByFunction")))),
                hasDescendant(cxxOperatorCallExpr(hasDeclaration(
                    cxxMethodDecl(
                        anyOf(matchesName(usedMethodName),
                              hasName(usedMethodName)),
                        ofClass(cxxRecordDecl(matchesName(usedClassName),
...
DeclarationMatcher methodWithReferencedMethod_Matcher(string mainMethodName,
  ...
  DeclarationMatcher matcher =
      cxxMethodDecl(
          unless(isImplicitFunction()),
          anyOf(forEachDescendant(memberExpr(hasDeclaration(
                    cxxMethodDecl(
                        anyOf(matchesName(usedMethodName),
                              hasName(usedMethodName)),
                        ofClass(cxxRecordDecl(matchesName(usedClassName),
                        ...
                        .bind("usedMethod")))),
                forEachDescendant(cxxOperatorCallExpr(hasDeclaration(
                    cxxMethodDecl(
                        anyOf(matchesName(usedMethodName),
                              hasName(usedMethodName)),
                        ofClass(cxxRecordDecl(matchesName(usedClassName),
          ...
          ofClass(cxxRecordDecl(matchesName(mainClassName),
                                unless(isTemplateInstantiation()))
...
DeclarationMatcher noExceptMethod_Matcher(string methodName, string className) {
  DeclarationMatcher matcher =
      cxxMethodDecl(unless(isImplicitMethod()),
                    anyOf(matchesName(methodName), hasName(methodName)),
                    ofClass(cxxRecordDecl(matchesName(className),
                                          unless(isTemplateInstantiation()))
...
DeclarationMatcher deletedMethod_Matcher(string methodName, string className) {
  DeclarationMatcher matcher =
      cxxMethodDecl(anyOf(matchesName(methodName), hasName(methodName)),
                    isDeleted(), unless(isImplicitMethod()),
                    ofClass(cxxRecordDecl(matchesName(className),
                                          unless(isTemplateInstantiation()))
...
DeclarationMatcher defaultedMethod_Matcher(string methodName,
                                           string className) {
  DeclarationMatcher matcher =
      cxxMethodDecl(anyOf(matchesName(methodName), hasName(methodName)),
                    isDefaulted(), unless(isImplicitMethod()),
                    ofClass(cxxRecordDecl(matchesName(className),
                                          unless(isTemplateInstantiation()))
...
DeclarationMatcher virtualMethod_Matcher(string methodName, string className) {
  DeclarationMatcher matcher =
      cxxMethodDecl(anyOf(matchesName(methodName), hasName(methodName)),
                    isVirtual(), unless(isImplicitMethod()),
                    ofClass(cxxRecordDecl(matchesName(className),
                                          unless(isTemplateInstantiation()))
...
DeclarationMatcher copyAssignmentOperator_Matcher(string className) {
  DeclarationMatcher matcher =
      cxxMethodDecl(isCopyAssignmentOperator(), unless(isDefaulted()),
                    unless(isImplicit()),
                    ofClass(cxxRecordDecl(matchesName(className),
                                          unless(isTemplateInstantiation()))
...
DeclarationMatcher moveAssignmentOperator_Matcher(string className) {
  DeclarationMatcher matcher =
      cxxMethodDecl(isMoveAssignmentOperator(), unless(isDefaulted()),
                    unless(isImplicit()),
                    ofClass(cxxRecordDecl(matchesName(className),
                                          unless(isTemplateInstantiation()))
...
DeclarationMatcher functionWithReferencedFuntion_Matcher(
  ...
  DeclarationMatcher matcher =
      functionDecl(matchesName(mainFunctionName),
                   hasDescendant(declRefExpr(
                       to(functionDecl(matchesName(usedFunctionName))
                              .bind("usedFunction")))),
                   unless(isImplicitFunction()))
          .bind("mainFunction");
  return matcher;
...
DeclarationMatcher methodWithReferencedFuntion_Matcher(
    string mainMethodName, string mainClassName, string usedFunctionName) {
  DeclarationMatcher matcher =
      cxxMethodDecl(unless(isImplicitFunction()),
                    hasDescendant(declRefExpr(
                        to(functionDecl(matchesName(usedFunctionName))
...
DeclarationMatcher functionName_Matcher(string functionName) {
  DeclarationMatcher matcher =
      functionDecl(matchesName(functionName), unless(isImplicitFunction()))
...
DeclarationMatcher invocationsFromHeader_Matcher(string nameFunction) {
  DeclarationMatcher matcher = functionDecl(
      ...
      forEachDescendant(
          declRefExpr(
              to(functionDecl(hasName(nameFunction)).bind("functionCalled")))
              .bind("refFunction")));
  return matcher;
}
