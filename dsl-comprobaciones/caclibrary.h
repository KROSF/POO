...
class checkCode : public MatchFinder::MatchCallback {
 public:
  checkCode(int argc, const char **argv, string fileClass = "",
            string help = "Comando: ./main (*).cpp -- -std=c++11");
...
- Utility: Method for setting the message that will be shown at the end of the
check when all the checks have passed.
...
- Utility: Method for setting the message that will be shown at the end of the
check when at least one check has failed.
...
- Utility: Method that has to be called after indicating a set of checks to
execute them.
*/
  void check();
...
followed by the operator without leaving any spaces in between. For example
"operator+=".
...
all the methods with the same name in the indicated class (regardless of their
parameters).

In the descriptions of the checks in this file, this property is indicated with
(?).

Note: In the case of setting "?" as argument of a parameter to indicate the list
of parameters of a method, the check will not take into account the parameter
list. We have to distinguish between this case and the situation of a method
without parameters; that case is indicated with an empty list {}.
...
For example, if "A.*" is set as the name of a class, the check will apply to all
the classes whose name starts with "A".
...
- Parameters:
---vector<string> className (*): vector of strings with the name of the classes
to be searched.
...
- Utility: Method to check the existence of a class. Unlike the check
"findClass", this check is executed when the method "check" is invoked.
...
*/
  ...
  /*
- Method: void listInitializerConstructor(string className, vector<string>
parameters, vector<string> initializers, string message)

- Utility: Method to verify that a class has a constructor that makes use of an
initializer list.

- Parameters:
---string className (*): string with the name of the class to be searched.
---vector<string> parameters (?): vector of strings with the types of the
parameters of the constructor.
---vector<string> initializers (?): vector of strings with the types of the
attributes that are initialized in the initializer list of the constructor (the
order of the initilizers is unimportant).
...
*/
  void listInitializerConstructor(string className, vector<string> parameters,
                                  ...
                                  string message =
                                      "CLASS WITH THE SPECIFIED CONSTRUCTOR "
                                      "AND INITIALIZATION LIST NOT FOUND.");
...
*/
  ...
  /*
- Method: void destructorDeleteMember(string className, vector<string>
memberNames, string message)
...
*/
  void destructorDeleteMember(
      string className, vector<string> memberNames,
      string message = "MEMBER VARIABLE NOT DELETED IN DESTRUCTOR.");
...
- Parameters:
---string className (*): string with the name of the class to be searched.
---unsigned int constructors: integer with the number of consrtuctor of the
class.
---lenient: boolean indicating whether it is allowed a number of constructors
equal or greater (true) or only an equal number of constructors (false). By
...
*/
  void numberOfConstructors(
      string className, unsigned int constructors, bool lenient = false,
      string message = "INCORRECT NUMBER OF CONSTRUCTORS.");

  /*
- Method: void methodWithDynamicCast(string methodName, vector<string>
parameters, string className, string constant, unsigned int numberdc, bool
lenient, string message)

- Utility: Method to verify that a method uses a dynamic cast with an origin and
destination types.

- Parameters:
---string methodName (*): string with the name of the method that should use the
dynamic cast.
---vector<string> parameters: vector of strings with the parameters of the
...
*/
  void methodWithDynamicCast(
      string methodName, vector<string> parameters, string className,
      string constant, string originType, string dstType,
      string message = "INCORRECT NUMBER OF DYNAMIC_CAST");

  /*
- Method: void explicitSpecifiedConstructor(string className, vector<string>
...
*/
  void explicitSpecifiedConstructor(
      string className, vector<string> parameters,
      string message = "EXPLICIT CONSTRUCTOR NOT FOUND.");
...
*/
  void takeException(vector<string> exceptionClasses,
                     string message = "EXCEPTION NOT FOUND");
...
vector<string> constant, vector<bool> exist, string message)
...
---vector<string> constant (?): vector of strings indicating wheter the
attributes are constant or not.
---vector<bool> exist: vector of boolean indicating the existence of the
attributes.
...
- Utility: Method to verify that all the variables created with "new", are later
deleted with "delete".
...
*/
  ...
  /*
- Method: void memberVariableAccessLevel(string className, vector<string>
memberNames, string level, string message)

- Utility: Method to verify that a class has an attribute with a specific access
level.

Parameters:
---string className (*): string with the name of the class to be searched.
---vector<string> memberName (*): vector of strings with the names of attributes
to be searched.
...
*/
  void memberVariableAccessLevel(
      string className, vector<string> memberNames, string level,
      string message = "MEMBER VARIABLE WITH THAT ACCESS LEVEL NOT FOUND.");
  ...
  /*
- Method: void method(vector<string> methodNames, vector<vector<string> >
parameters, string className, vector<string> constant,string message)

- Utility: Method to verify the existence of a method.

- Parameters:
---vector<string> methodNames (*): vector of strings with the names of the
methods to be searched.
---vector<vector<string> > parameters (?): vector of vector of strings with the
types of the method parameters to be searched.
---string className (*): string with the name of the class where the methods
have to be searched.
---vector<string> constant (?): vector of strings indicating whether the methods
are constant or not.
...
- Example:
  ...
  method({"method1", "method2"}, {{"Type1", "Type2"}, {}}, "A", {"?", "?"});
Pass method({"method3"}, {{"?"}}, "A"); Fail
...
vector<vector<string> > parameters, string className, vector<string> constant,
string usedVariable, string message)
...
---vector<string> constant (?): vector of strings indicating whether the methods
are constant or not.
---vector<string> usedVariables: vector of strings with the names of the
attributes that are referenced by the methods.
...
- Example:
  ...
  methodWithReferencedMemberVariable({"method1"}, {{"?"}}, "A", {"?"}, {"a"});
Pass methodWithReferencedMemberVariable({"method2"}, {{"?"}}, "A", {"?"},
{"a"}); Fail

*/
  void methodWithReferencedMemberVariable(
      ...
      string message = "METHOD WITH REFERENCED MEMBER VARIABLE NOT FOUND.");
...
- Parameters:
---vector<string> mainMethodNames (*): vector of strings with the names of the
main methods to be searched.
...
---vector<string> usedMethodNames (*): vector of strings with the name of the
methods to be searched that are referenced by the main methods.
---vector<vector<string> > usedMethodparameters (?): vector of vector of strings
with the types of the parameters of the referenced methods.
---string usedClassName (*): string with the name of the class where the
referenced methods have to be searched.
---vector<string> usedConstant (?): vector of strings indicating whether the
referenced methods are constant or not.
...
- Example:
  ...
  methodWithReferencedMethod({"method1"}, {{"?"}}, "A", {"?"}, {"method3"},
{{"?"}}, "A", {"?"}); Pass methodWithReferencedMethod({"method2"}, {{"?"}}, "A",
{"?"}, {"method3"}, {{"?"}}, "A", {"?"}); Fail

*/
  void methodWithReferencedMethod(
      vector<string> mainMethodNames,
      vector<vector<string>> mainMethodparameters, string mainClassName,
      vector<string> mainConstant, vector<string> usedMethodName,
      vector<vector<string>> usedMethodparameters, string usedClassName,
      vector<string> usedConstant,
      string message = "METHOD WITH REFERENCED METHOD NOT FOUND.");

  /*
- Method:  void noExceptMethod(vector<string> methodNames, vector<vector<string>
> parameters, string className, vector<string> constant, string message)
...
---vector<string> methodNames (*): vector of strings with the names of the
methods to be searched.
---vector<string> parameters (?): vector of vector of strings with the types of
the parameters of the methods to be searched.
---string className (*): string with the name of the class where the methods
have to be searched.
---vector<string> constant (?): vector of strings indicating whether the methods
are constant or not.
...
*/
  void noExceptMethod(vector<string> methodNames,
                      ...
                      vector<string> constant = {"?"},
                      string message = "METHOD NO EXCEPT NOT FOUND.");

  /*
- Method:  void inlineMethod(vector<string> methodNames, vector<vector<string> >
parameters, string className, vector<string> constant, string message)

- Utility: Method to verify that a method is marked as "inline"

- Parameters:
---vector<string> methodNames (*): vector of strings with the name of the
methods to be searched.
---vector<string> parameters (?): vector of vector of strings with the types of
the parameters of the methods to be searched.
---string className (*): string with the name of the class where the methods
...
- Example:
  ...
  inlineMethod({"method1"}, {{"?"}}, "A", {"?"}); Pass inlineMethod({"method1",
method2"}, {{"?"}, {"?"}}, "A", {"?","?"});       Pass inlineMethod({"method3"},
{{"?"}}, "A", {"?"});                            Fail inlineMethod({"method2",
method3"}, {{"?"}, {"?"}}, "A", {"?","?"});       Fail
...
vector<vector<string> > parameters, string className, vector<string> constant,
vector<unsigned int> defaultArgs, string message)

- Utility: Method to verify that a method of a class has a number of default
arguments.
...
---vector<string> constant (?): vector of strings indicating whether the methods
are constant or not.
---vector<unsigned int> numDefaultArgs: vector of integers indicating the number
of default arguments that the methods should have.
---vector<vector<string> > defaultArgs (*, ?): vector of vector of strings
indicating the default arguments.
...
- Example:
  ...
  defaultArgumentsInMethod({"method2"}, {{"Type1", "Type2"}}, "A", {"?"}, {2});
Fail defaultArgumentsInMethod({"method2"}, {{"Type1", "Type2"}}, "A", {"?"},
{1}, {"0.*"}); Pass defaultArgumentsInMethod({"method2"}, {{"Type1", "Type2"}},
"A", {"?"}, {1}, {"?"});   Pass

*/
  void defaultArgumentsInMethod(
      vector<string> methodNames, vector<vector<string>> parameters,
      string className, vector<string> constant = {"?"},
      vector<unsigned int> numDefaultArgs = {0},
      vector<vector<string>> defaultArgs = {{"?"}},
      string message = "DEFAULT ARGUMENTS IN METHODS NOT FOUND.");

  /*
- Method:  void deletedMethod(vector<string> methodNames, vector<vector<string>>
parameters, string className, vector<string> constant, string message);
...
*/
  ...
  /*
- Method:  void virtualMethod(vector<string> methodNames, vector<vector<string>>
parameters, string className, vector<string> constant, string message);
...
*/
  ...
  /*
- Method: void copyAssignmentOperator(string className, bool exist, string
message)
...
*/
  void copyAssignmentOperator(
      string className, bool exist,
      string message = "COPY ASSIGNMENT OPERATOR NOT FOUND.");

  /*
- Method: void moveAssignmentOperator(string className, bool exist, string
message)
...
*/
  void moveAssignmentOperator(
      string className, bool exist = true,
      string message = "MOVE ASSIGNMENT OPERATOR NOT FOUND.");
  ...
  /*
- Method:  void function(vector<string> functionNames, vector<vector<string> >
parameters, string message)

- Utility: Method to verify the existence of a function

- Parameters:
---vector<string> functionNames (*): vector of strings with the name of the
functions to be searched.
---vector<vector<string> > parameters (?): vector of vector of strings with the
types of the parameters of the functions to be searched.
...
*/
  void function(vector<string> functionNames, vector<vector<string>> parameters,
                string message = "FUNCTION NOT FOUND.");

  /*
- Method: void methodWithReferencedFunction(vector<string> mainMethodNames,
vector<vector<string> > mainMethodparameters, string mainClassName,
vector<string> mainConstant, vector<string> usedFunctionNames,
...
main methods to be searched.
---vector<vector<string> > mainMethodparameters (?): vector of vector of strings
with the names of the types of the parameters with the main methods to be
searched.
---string mainClassName (*): string with the name of the class where the main
methods have to be searched.
---vector<string> mainConstant (?): vector of strings indicating whether the
main methods are constant or not.
---vector<string> usedFunctionNames (*): vector of strings with the names of the
functions to be searched that are referenced by the main functions.
---vector<vector<string> > usedFunctionParameters (?): vector of vector strings
with the types of the parameters of the referenced functions.
...
- Example:
  ...
  methodWithReferencedFunction({"method1"}, {{"?"}}, "A", {"?"}, {"function1"},
{{"?"}}); Pass methodWithReferencedFunction({"method1"}, {{"?"}}, "A", {"?"},
{"function2"}, {{"?"}}); Fail


*/
  void methodWithReferencedFunction(
      vector<string> mainMethodNames,
      vector<vector<string>> mainMethodparameters, string mainClassName,
      vector<string> mainConstant, vector<string> usedFunctionNames,
      vector<vector<string>> usedFunctionParameters,
      string message = "FUNCTION WITH REFERENCED FUNCTION NOT FOUND.");
  /*
...
- Parameters:
---vector<string> mainFunctionNames (*): vector of strings with the names of the
main functions to be searched.
---vector<vector<string> > mainFunctionParameters (?): vector of vector of
strings with the types of the parameters of the main functions to be searched.
---vector<string> usedMethodNames (*): vector of strings with the name of the
method to be searched that are referenced by the main method.
---vector<vector<string> > usedMethodparameters (?): vector of vector of strings
with the types of the parameters of the referenced methods to be searched.
---string usedClassName (*): string with the name of the class where the
referenced methods have to be searched.
---vector<string> usedConstant (?): vector of strings indicating whether the
referenced methods are constant or not.
...
"A", {"?"}); Pass functionWithReferencedMethod({"function1"}, {{"?"}},
{"method2"}, {{"?"}}, "A", {"?"}); Fail


*/
  void functionWithReferencedMethod(
      vector<string> mainFunctionNames,
      vector<vector<string>> mainFunctionParameters,
      vector<string> usedMethodNames,
      vector<vector<string>> usedMethodparameters, string usedClassName,
      vector<string> usedConstant,
      string message = "FUNCTION WITH REFERENCED METHOD NOT FOUND.");

  /*
- Method: void functionWithReferencedFunction(vector<string> mainFunctionNames,
vector<vector<string> > mainFunctionParameters, vector<string>
usedFunctionNames, vector<vector<string> > usedFunctionParameters, string
message)
...
---vector<string> usedFunctionNames (*): vector of strings with the names of the
functions to be searched that are referenced by the main functions.
---vector<vector<string> > usedFunctionParameters (?): vector of vector of
strings with the types of the parameters of the referenced functions to be
searched.
...
{{"?"}}); Pass functionWithReferencedFunction({"function2"}, {{"?"}},
{"function3"}, {{"?"}}); Fail


*/
  void functionWithReferencedFunction(
      vector<string> mainFunctionNames,
      vector<vector<string>> mainFunctionParameters,
   ...
   ****************************************************************************/
  /*
- Method:  void invocationsFromHeaders(vector<string> functionNames, string
headerName, bool exist, string message);

- Utility: Method to verify whether the correct header has been included when
certain functions are referenced.

- Parameters:
---vector<string> functionNames: vector of strings with the name of the
functions to be searched.
---string headerName: string with the name of the header.
---bool exist: boolean indicating that the functions should be taken from the
...
*/
  void invocationsFromHeaders(
      vector<string> functionNames, string headerName, bool exist = true,
      string message = "A PROBLEM WITH THE INCLUDED HEADERS WAS FOUND.");

  /*
- Method:  void fileIncludedHeader(string headerName, vector<string> fileNames,
string message);

- Utility: Method to verify that a header has been included in any of the
indicated files.

- Parameters:
---string headerName: string with the name of the header to be searched.
---vector<string> fileNames: vector of strings with the name of the files where
the header has to be searched.
...
*/
  void fileIncludedHeader(
      string headerName, vector<string> fileNames,
      string message = "HEADER IS NOT INCLUDED IN ANY OF THE FILES");
...
- Parameters:
---string fileName: string with the name of the file where the guard clause has
to be searched.
...
- Utility: Method to verify that a function has been marked as friend of a
class.

- Parameters:
---vector<string> functionNames (*): vector of strings with the names of the
functions to be searched.
---vector<vector<string> > parameters (?): vector of vector of strings with the
types of the parameters of the functions to be searched.
---string className (*): string with the name of the class where the desired
functions have to be searched.
...
- Utility: Method to verify that a class has been marked as friend of another
one.

- Parameters:
---string className (*): string with the name of the class where the friend
classes has to be searched.
---vector<string> friendClassesNames (*): vector of strings with the names of
the friend classes to be searched.
