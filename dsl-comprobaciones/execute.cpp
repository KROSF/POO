...
bool execute::existConstructor(const CXXConstructorDecl *foundConstructor) {
  if (foundConstructor && foundConstructor->isThisDeclarationADefinition()) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundConstructor->getLocStart());
    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
      return true;
    } else {
...
bool execute::existDestructor(const CXXDestructorDecl *foundDestructor) {
  if (foundDestructor) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundDestructor->getLocStart());
    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
      return true;
    } else {
...
bool execute::existDeleteExpression(
    const CXXDeleteExpr *foundDeleteExpression) {
  if (foundDeleteExpression) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundDeleteExpression->getLocStart());
    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
      return true;
    } else {
...
bool execute::existMethod(const CXXMethodDecl *foundMethod) {
  if (foundMethod && foundMethod->isThisDeclarationADefinition()) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundMethod->getLocStart());
    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
      return true;
    } else {
...
bool execute::existFunction(const FunctionDecl *foundFunction) {
  if (foundFunction && foundFunction->isThisDeclarationADefinition()) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundFunction->getLocStart());
    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
      return true;
    } else {
...
bool execute::existMember(const FieldDecl *foundMember) {
  if (foundMember) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundMember->getLocStart());
    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
      return true;
    } else {
...
bool execute::existVariable(const VarDecl *foundVariable) {
  if (foundVariable) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundVariable->getLocStart());
    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
      return true;
    } else {
...
bool execute::checkInitializersConstructor(
    const CXXConstructorDecl *foundConstructor, vector<string> parameters) {
  bool idem = true;
  if (foundConstructor->getNumCtorInitializers() == parameters.size()) {
    unsigned int p = 0;
    for (CXXConstructorDecl::init_const_iterator
             I = foundConstructor->init_begin(),
             E = foundConstructor->init_end();
         I != E && idem; ++I) {
...
bool execute::checkParamsFunction(const FunctionDecl *foundFunction,
  ...
  if (foundFunction->getNumParams() == parameters.size()) {
    for (unsigned int p = 0; p < foundFunction->getNumParams(); p++) {
      // llvm::outs() << foundFunction->getNameAsString() << ": ";
      // llvm::outs()<<
      // foundFunction->getParamDecl(p)->getType().getCanonicalType().getAsString()
      // << "-"; llvm::outs() << parameters[p];
      if (foundFunction->getParamDecl(p)
              ->getType()
              .getCanonicalType()
...
string execute::getCorrectMessage() { return correctMessage; }

string execute::getIncorrectMessage() { return incorrectMessage; }
...
void execute::setMethodsWithReferencedMethod(
    string mainMethodName, vector<string> mainMethodParams,
    string mainClassName, string mainConstant, string usedMethodName,
    vector<string> usedMethodParams, string usedClassName, string usedConstant,
    string message) {
...
void execute::setDefaultArgumentsInMethod(
    string methodName, vector<string> params, string className, string constant,
    unsigned int numDefaultArgs, vector<string> defaultArgs, string message) {
  Info info(className, message, false);
  info.setClass(className);
  info.setParameters(params);
  info.setConstant(constant);
...
void execute::setFunctionsWithReferencedFunction(
    ...
    string usedFunctionName, vector<string> usedFunctionParameters,
    string message) {
  ...
  functionsWithReferencedFunction.insert(
      std::make_pair(mainFunctionName, info));
}

void execute::setMethodsWithReferencedFunction(
    string mainMethodName, vector<string> mainMethodParams,
    string mainClassName, string mainConstant, string usedFuncionName,
    vector<string> usedFunctionParams, string message) {
  Info info(mainMethodName, message, false);
  info.setClass(mainClassName);
  info.setParameters(mainMethodParams);
...
void execute::setGuardClauses(string fileName, string message) {
  ...
  if (codeFile.is_open()) {
    ...
    code = regex_replace(code, comment_regex,
                         "");  // Use gcc-4.9 or any later version for proper
    ...
    if (!token.empty()) {
      ...
      // Finally: Check if the next word coincides with the token previously
      // saved. If so, search for "#endif"
      if (it != end && *it == token)
        while (it != end && !found) {
          found = *(it) == "#endif";
          ++it;
...
void execute::setCorrectMessage(string message) { correctMessage = message; }
...
void execute::setDynamicCast(string methodName, vector<string> parameters,
  ...
  info.setParameters(parameters);
  info.setConstant(constant);
  // The attributes memberVariable and usedClass in the class Info are reused
  // instead of defining new attributes
...
void execute::classWithDefaultConstructor(
  ...
  if (existClass(foundClassWithDefaultConstructor) &&
    ...
    for (AI it = defaultConstructors.begin(); it != defaultConstructors.end();
         ++it) {
...
void execute::classWithListInitializerConstructor(
    const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithListInitializerConstructor) &&
    ...
    for (AI it = initializers.begin(); it != initializers.end(); ++it) {
      if (std::regex_match(
              foundClassWithListInitializerConstructor->getNameAsString(),
              std::regex(it->first))) {
...
void execute::classWithDestructor(const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithDestructor) &&
      existDestructor(foundDestructor)) {
...
void execute::explicitSpecifiedConstructor(
    const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithExplicitSpecifiedConstructor)) {
    for (AI it = explicitSpecifiedConstructors.begin();
      ...
      if (std::regex_match(
              foundClassWithExplicitSpecifiedConstructor->getNameAsString(),
              std::regex(it->first))) {
...
void execute::takeException(const MatchFinder::MatchResult &Result) {
  if (const CXXThrowExpr *foundException =
    ...
    FullSourceLoc FullLocation1 =
        Context->getFullLoc(foundException->getLocStart());
...
void execute::classWithAllPrivateMember(
    const MatchFinder::MatchResult &Result) {
...
void execute::classWithVariableMembersAccessLevel(
    const MatchFinder::MatchResult &Result) {
  const CXXRecordDecl *foundClass =
      Result.Nodes.getNodeAs<clang::CXXRecordDecl>("foundClassAccessLevel");
  const FieldDecl *foundMember = Result.Nodes.getNodeAs<clang::FieldDecl>(
      "foundMemberVariableAccessLevel");
  ...
  if (existClass(foundClass) && existMember(foundMember)) {
    for (AI it = variableMembersAccessLevel.begin();
      ...
      if (std::regex_match(foundMember->getNameAsString(),
        ...
        if (accessCorrect) (it->second).setFound(true);
      }
    }
  }
}
...
void execute::methodWithReferencedVariable(
    const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithMethodVariable) &&
    ...
    if (foundReferencedVariable) {
      for (AI it = methodsWithReferencedVariable.begin();
        ...
        if (checkRegularExpresion(
                foundClassWithMethodVariable->getNameAsString(),
                (it->second).getClass()) &&
            checkRegularExpresion(foundReferencedVariable->getNameAsString(),
                                  (it->second).getMemberVariable())) {
          if (checkRegularExpresion(
                  foundMethodWithReferencedVariable->getNameAsString(),
                  it->first)) {
...
void execute::methodWithReferencedMethod(
    const MatchFinder::MatchResult &Result) {
    ...
    if (existClass(foundClassOfReferencedMethod) &&
      ...
      for (AI it = methodsWithReferencedMethod.begin();
        ...
        if (checkRegularExpresion(foundMainMethod->getNameAsString(),
                                  it->first) &&
            checkRegularExpresion(
                foundClassWithReferencedMethod->getNameAsString(),
                (it->second).getClass())) {
          if (checkParams(foundMainMethod, (it->second).getParameters())) {
            if (markedConst((it->second).getConstant(),
              ...
              if (checkRegularExpresion(foundUsedMethod->getNameAsString(),
                  ...
                  checkRegularExpresion(
                      foundClassOfReferencedMethod->getNameAsString(),
...
void execute::inlineMethod(const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithInlineMethod) &&
      existMethod(foundInlineMethod) == true) {
    if (foundInlineMethod->isInlined()) {
      for (AI it = inlineMethods.begin(); it != inlineMethods.end(); ++it) {
        if (checkRegularExpresion(foundInlineMethod->getNameAsString(),
                                  it->first) &&
            checkRegularExpresion(foundClassWithInlineMethod->getNameAsString(),
                                  (it->second).getClass())) {
  ...
  if (existClass(foundClassWithDefArgsInMethod) &&
    ...
    for (AI it = defaultArgumentsInMethods.begin();
      ...
      if (checkRegularExpresion(foundDefArgsInMethod->getNameAsString(),
                                it->first) &&
          checkRegularExpresion(
              foundClassWithDefArgsInMethod->getNameAsString(),
              (it->second).getClass())) {
...
void execute::defaultedMethod(const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithDefaultedMethod) && foundDefaultedMethod) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundDefaultedMethod->getLocStart());

    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
      for (AI it = defaultedMethods.begin(); it != defaultedMethods.end();
           ++it) {
        if (checkRegularExpresion(foundDefaultedMethod->getNameAsString(),
                                  it->first) &&
            checkRegularExpresion(
                foundClassWithDefaultedMethod->getNameAsString(),
                (it->second).getClass()) &&
...
// ----------------------- CHECK OPERATORS ---------------------
void execute::classWithCopyAssignmentOperator(
    const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithCopyAssignmentOperator) &&
    ...
    for (AI it = copyAssignmentOperators.begin();
      ...
      if (std::regex_match(
              foundClassWithCopyAssignmentOperator->getNameAsString(),
              std::regex(it->first))) {
        (it->second).setFound(true);
      }
    }
...
void execute::classWithMoveAssignmentOperator(
    const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithMoveAssignmentOperator) &&
    ...
    for (AI it = moveAssignmentOperators.begin();
      ...
      if (std::regex_match(
              foundClassWithMoveAssignmentOperator->getNameAsString(),
              std::regex(it->first))) {
        (it->second).setFound(true);
      }
    }
...
void execute::functionWithReferencedFunction(
    const MatchFinder::MatchResult &Result) {
...
void execute::methodWithReferencedFunction(
    const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithReferencedFunction) &&
    ...
    if (existFunction(foundUsedFunction)) {
      for (AI it = methodsWithReferencedFunction.begin();
           it != methodsWithReferencedFunction.end(); ++it) {
        if (checkRegularExpresion(foundMainMethod->getNameAsString(),
                                  it->first) &&
            checkRegularExpresion(
                foundClassWithReferencedFunction->getNameAsString(),
                (it->second).getClass())) {
          if (checkParams(foundMainMethod, (it->second).getParameters())) {
            if (markedConst((it->second).getConstant(),
                            foundMainMethod->isConst())) {
...
void execute::functionWithReferencedMethod(
    const MatchFinder::MatchResult &Result) {
  const FunctionDecl *foundMainFunction =
...
void execute::functionWithName(const MatchFinder::MatchResult &Result) {
  ...
  if (existFunction(foundFunctionWithName)) {
    for (AI it = functionsWithName.begin(); it != functionsWithName.end();
         ++it) {
      if (checkRegularExpresion(foundFunctionWithName->getNameAsString(),
                                it->first)) {
        // llvm::outs() <<
        // foundFunctionWithName->getType().getCanonicalType().getAsString() <<
        // "\n";
    ...
    if (FullLocation1.isValid()) {
      ...
      for (AI i = rango.first; i != rango.second; ++i) {
        if (FullLocation2.isValid()) {
          ...
          if (string(Context->getSourceManager()
                         .getFileEntryForID(FullLocation2.getFileID())
                         ->getName())
...
void execute::classWithFriendFunction(const MatchFinder::MatchResult &Result) {
  ...
  if (foundFriendFunction && foundClassWithFriendFunction) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundFriendFunction->getLocStart());
    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
      for (AI it = friendFunctions.begin(); it != friendFunctions.end(); ++it) {
        if (checkRegularExpresion(foundFriendFunction->getNameAsString(),
                                  it->first)) {
          if (checkRegularExpresion(
                  foundClassWithFriendFunction->getNameAsString(),
                  (it->second).getClass())) {
            if (checkParams(foundFriendFunction,
                            (it->second).getParameters())) {
              (it->second).setFound(true);
            }
          }
...
void execute::classWithFriendClass(const MatchFinder::MatchResult &Result) {
  ...
  if (foundClassWithFriendClass) {
    FullSourceLoc FullLocation =
        Context->getFullLoc(foundClassWithFriendClass->getLocStart());
    if (FullLocation.isValid() &&
        !Context->getSourceManager().isInSystemHeader(FullLocation)) {
...
bool execute::friend_class(const CXXRecordDecl *c1, string c2) {
  for (CXXRecordDecl::friend_iterator fi = c1->friend_begin();
    ...
    if ((*fi)->getFriendType() != NULL) {
      ...
      //			llvm::outs() <<
      //qt->getAsCXXRecordDecl()->getNameAsString();
      //	}
...
void execute::methodWithDynamicCast(const MatchFinder::MatchResult &Result) {
  ...
  if (existClass(foundClassWithDynamicCast) &&
      existMethod(foundMethodWithDynamicCast) && foundDynamicCast != nullptr &&
      dynamicType != nullptr) {
