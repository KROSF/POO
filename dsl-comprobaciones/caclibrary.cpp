...
checkCode::checkCode(int argc, const char **argv, string fileClass,
                     string help) {
  setHelper(help);
  if (argc < 2) {
  ...
  string command =
      "echo | clang -E -v -x c++ - 2>&1 | sed -n '/<...> search starts "
      "here/,/End of search list./p' | sed 's/End of search list.//g' | sed "
      "'s/#include <...> search starts here://g' >" +
      file_name;

  list<string> lstr = headers_read(file_name, command);

  command =
      "echo | LANG=C cpp -v -x c++ - 2>&1 | sed -n '/<...> search starts "
      "here/,/End of search list./p' | sed 's/End of search list.//g' | sed "
      "'s/#include <...> search starts here://g' >" +
      file_name;
...
void checkCode::check() {
  int i = 0;
  for (std::vector<string>::iterator it = argvs.begin(); it != argvs.end();
       ++it) {
    argv2[i] = (*it).c_str();
    i++;
  }
...
MatchFinder checkCode::getFinder() { return Finder; }

string checkCode::getHelper() { return helper; }
...
void checkCode::setHelper(string help) { helper = help; }
...
void checkCode::apply_class(const MatchFinder::MatchResult &Result) {
  if (const clang::CXXRecordDecl *Node =
    ...
    if (it != classes.end()) it->second = true;
  }
}
...
bool checkCode::findClass(vector<string> className) {
  ...
  if (validFile == true) {
    ...
    for (vector<string>::iterator it = className.begin(); it != className.end();
         ++it) {
...
bool checkCode::foundClass() { return class_found; }
...
void checkCode::methodWithDynamicCast(string methodName,
                                      ...
                                      string message) {
  e.setDynamicCast(methodName, parameters, constant, className, originType,
                   dstType, message);
...
void checkCode::memberVariableAccessLevel(string className,
  ...
  for (unsigned int i = 0; i < memberNames.size(); i++) {
    ...
    Finder.addMatcher(
        memberVariableAccessLevel_Matcher(className, memberNames[i]), &e);
  }
}
...
void checkCode::method(vector<string> methodNames,
  ...
  if (methodNames.size() == parameters.size()) {
    ...
    llvm::outs() << "checkCode::method => different number of methodNames and "
                    "parameters.\n";
  }
}
...
void checkCode::methodWithReferencedMemberVariable(
  ...
  if (methodNames.size() == parameters.size() &&
    ...
    for (unsigned int i = 0; i < methodNames.size(); i++) {
      ...
      Finder.addMatcher(methodWithReferencedVariable_Matcher(
                            methodNames[i], className, usedVariables[i]),
                        &e);
    }
  } else {
    llvm::outs()
        << "checkCode::methodWithReferencedMemberVariable => different number "
           "of methodNames, parameters and usedVariables.\n";
  }
}
...
void checkCode::noExceptMethod(vector<string> methodNames,
  ...
  if (methodNames.size() == parameters.size()) {
    ...
    llvm::outs() << "checkCode::noExceptMethod => different number of "
                    "methodNames and parameters.\n";
  }
}
...
void checkCode::inlineMethod(vector<string> methodNames,
  ...
  if (methodNames.size() == parameters.size()) {
    for (unsigned int i = 0; i < methodNames.size(); i++) {
      e.setInlineMethods(methodNames[i], parameters[i], className, constant[i],
                         message);
      Finder.addMatcher(inlineMethod_Matcher(methodNames[i], className), &e);
    }
  } else {
    llvm::outs() << "checkCode::inlineMethod => different number of "
                    "methodNames and parameters.\n";
  }
}
...
void checkCode::defaultArgumentsInMethod(vector<string> methodNames,
  ...
  if (methodNames.size() == parameters.size()) {
    for (unsigned int i = 0; i < methodNames.size(); i++) {
      if (numDefaultArgs[i] == 0 ||
          numDefaultArgs[i] == defaultArgs[i].size()) {
        e.setDefaultArgumentsInMethod(methodNames[i], parameters[i], className,
                                      constant[i], numDefaultArgs[i],
                                      defaultArgs[i], message);
        Finder.addMatcher(
            defaultArgumentsInMethod_Matcher(methodNames[i], className), &e);
      } else {
        llvm::outs() << "checkCode::defaultArgumentsInMethod => numDefaultArgs "
                        "and defaultArgs do not match.\n";
      }
    }
  } else {
    llvm::outs() << "checkCode::defaultArgumentsInMethod => different number "
                    "of methodNames and parameters.\n";
  }
}
...
void checkCode::deletedMethod(vector<string> methodNames,
  ...
  if (methodNames.size() == parameters.size()) {
    for (unsigned int i = 0; i < methodNames.size(); i++) {
      e.setDeletedMethods(methodNames[i], className, parameters[i], constant[i],
                          message);
      Finder.addMatcher(deletedMethod_Matcher(methodNames[i], className), &e);
    }
  } else {
    llvm::outs() << "checkCode::deletedMethod => different number of "
                    "methodNames and parameters.\n";
  }
}
...
void checkCode::defaultedMethod(vector<string> methodNames,
  ...
  if (methodNames.size() == parameters.size()) {
    ...
    llvm::outs() << "checkCode::defaultedMethod => different number of "
                    "methodNames and parameters.\n";
  }
}
...
void checkCode::virtualMethod(vector<string> methodNames,
  ...
  if (methodNames.size() == parameters.size()) {
    for (unsigned int i = 0; i < methodNames.size(); i++) {
      e.setVirtualMethods(methodNames[i], className, parameters[i], constant[i],
                          message);
...
void checkCode::methodWithReferencedMethod(
  ...
  if (mainMethodNames.size() == mainMethodparameters.size() &&
    ...
    for (unsigned int i = 0; i < mainMethodNames.size(); i++) {
      e.setMethodsWithReferencedMethod(
          mainMethodNames[i], mainMethodparameters[i], mainClassName,
          mainConstant[i], usedMethodNames[i], usedMethodparameters[i],
          usedClassName, usedConstant[i], message);
      Finder.addMatcher(
          methodWithReferencedMethod_Matcher(mainMethodNames[i], mainClassName,
                                             usedMethodNames[i], usedClassName),
          &e);
    }
  } else {
    llvm::outs()
        << "checkCode::methodWithReferencedMethod => different number of "
           "methodNames and parameters in the main and used method.\n";
  }
}
  ...
  if (mainMethodNames.size() == mainMethodparameters.size() &&
    ...
    for (unsigned int i = 0; i < mainMethodNames.size(); i++) {
      e.setMethodsWithReferencedFunction(mainMethodNames[i],
                                         ...
                                         mainConstant[i], usedFunctionNames[i],
                                         usedFunctionParameters[i], message);
      Finder.addMatcher(
          methodWithReferencedFuntion_Matcher(mainMethodNames[i], mainClassName,
                                              usedFunctionNames[i]),
          &e);
    }
  } else {
    llvm::outs()
        << "checkCode::methodWithReferencedFunction => different number of "
           "functionNames and parameters in the main and used function.\n";
  }
}
...
void checkCode::functionWithReferencedMethod(
  ...
  if (mainFunctionNames.size() == mainFunctionParameters.size() &&
    ...
    for (unsigned int i = 0; i < mainFunctionNames.size(); i++) {
      e.setFunctionsWithReferencedMethod(
          mainFunctionNames[i], mainFunctionParameters[i], usedMethodNames[i],
          usedMethodparameters[i], usedConstant[i], usedClassName, message);
      Finder.addMatcher(
          functionWithReferencedMethod_Matcher(
              mainFunctionNames[i], usedMethodNames[i], usedClassName),
          &e);
    }
  } else {
    llvm::outs()
        << "checkCode::functionWithReferencedMethod => different number of "
           "functionNames and parameters in the main and used function.\n";
  }
}
  ...
  if (mainFunctionNames.size() == mainFunctionParameters.size() &&
    ...
    for (unsigned int i = 0; i < mainFunctionNames.size(); i++) {
      ...
      Finder.addMatcher(functionWithReferencedFuntion_Matcher(
                            mainFunctionNames[i], usedFunctionNames[i]),
                        &e);
    }
  } else {
    llvm::outs()
        << "checkCode::functionWithReferencedFunction => different number of "
           "functionNames and parameters in the main and used function.\n";
  }
}
...
void checkCode::function(vector<string> functionNames,
  ...
  if (functionNames.size() == parameters.size()) {
    ...
    llvm::outs() << "checkCode::function => different number of functionNames "
                    "and parameters.\n";
  }
}
...
void checkCode::friendFunction(vector<string> functionNames,
  ...
  if (functionNames.size() == parameters.size()) {
    for (unsigned int i = 0; i < functionNames.size(); i++) {
      ...
      Finder.addMatcher(friendFunction_Matcher(functionNames[i], className),
                        &e);
    }
  } else {
    llvm::outs() << "checkCode::friendFunction => different number of "
                    "functionNames and parameters.\n";
  }
}
...
void checkCode::friendClass(string className, vector<string> friendClassesNames,
  ...
  for (unsigned int i = 0; i < friendClassesNames.size(); i++) {
    ...
    Finder.addMatcher(friendClass_Matcher(className, friendClassesNames[i]),
                      &e);
  }
}
