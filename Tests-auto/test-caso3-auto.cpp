...
FCTMF_FIXTURE_SUITE_BGN(test_p3_clases) {
  ...
  FCT_TEST_BGN(LineaPedido - constructor
               : valor por defecto en 2do. parametro) {
  ...
  FCT_TEST_BGN(Pedido - tarjeta caducada) {
    ...
    try {
      Pedido{*pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU, *pTarjetaU,
             fHoy + 30};
  ...
  FCT_TEST_BGN(Pedido - actualizacion relacionados y asociaciones) {
    ...
    const unique_ptr<const Pedido> pPed{new Pedido{
        *pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU, *pTarjetaU, fHoy}};
...
FCTMF_FIXTURE_SUITE_BGN(test_p3_informes) {
  ...
  FCT_SETUP_BGN() {
    ...
    pTarjetaU2 =
        new Tarjeta(TIPO::AmericanExpress, nTarjeta3, *pU2, fUnaSemana);

    // Primera venta
    pU->compra(articulo1, cantidad_A1_P1);
    pPed1 = new Pedido(*pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU,
                       *pTarjetaU, fHoy);

    // Segunda venta, de otro usuario
    pU2->compra(articulo1, cantidad_A1_P2);
    pU2->compra(articulo2, cantidad_A2_P2);
    pPed2 = new Pedido(*pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU2,
                       *pTarjetaU2, fHoy);
  };
  FCT_SETUP_END();
...
#ifndef CPP11REGEX
    const string sRegexA1P1(sRegexPrefijo + toString(cantidad_A1_P1) +
                            sRegexSufijo),
