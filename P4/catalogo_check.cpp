...
int main(int argc, const char **argv) {
  checkCode c1(
      argc, argv, "articulo.cpp",
      "Orden: ./pedido_check articulo.cpp tarjeta.cpp usuario.cpp pedido.cpp "
      "pedido-articulo.cpp usuario-pedido.hpp -- -std=c++14 -I../P1");

  c1.setCorrectMessage("Verificación correcta de la clase Artículo.");
  c1.setIncorrectMessage("REVISA LOS ERRORES DE ARTÍCULO.");

  vector<string> functionNames = {"strlen", "strcat", "memset", "strcpy",
                                  "strcmp"};
  string headerName = "cstring";

  if (c1.findClass({"Articulo", "ArticuloAlmacenable", "Autor", "Libro",
                    "Cederron", "LibroDigital"})) {
    ...
    c1.memberVariable("Articulo", {"stock_"}, {"?"}, {false},
                      "Revisa el enunciado respecto a los atributos que deben "
                      "estar en cada clase.");
    c1.memberVariable("ArticuloAlmacenable", {"stock_"}, {"noconst"}, {true},
                      "Revisa el enunciado respecto a los atributos que deben "
                      "estar en cada clase.");
    c1.virtualMethod(
        {"~Articulo"}, {{}}, "Articulo", {"noconst"},
        "Contempla crear un destructor apropiado para esta clase.");
    c1.functionWithReferencedMethod(
        {"operator<<"},
        {{"class std::basic_ostream<char> &", "const class Articulo &"}},
        {"impresion_especifica"}, {{"class std::basic_ostream<char> &"}},
        "Articulo", {"const"},
        "Incluya impresion_especifica en el operador de extracción");
    c1.methodWithReferencedMemberVariable(
        {"impresion_especifica"}, {{"class std::basic_ostream<char> &"}},
        "Libro", {"const"}, {"n_pag_"},
        "Revisa qué debe imprimir la definición del método "
        "impresion_especifica en cada clase.");
    c1.methodWithReferencedMemberVariable(
        {"impresion_especifica"}, {{"class std::basic_ostream<char> &"}},
        "Libro", {"const"}, {"stock_"},
        "Revisa qué debe imprimir la definición del método "
    ...
    c1.methodWithReferencedMemberVariable(
        {"impresion_especifica"}, {{"class std::basic_ostream<char> &"}},
        "Cederron", {"const"}, {"stock_"},
        "Revisa qué debe imprimir la definición del método "
        "impresion_especifica en cada clase.");
    c1.methodWithReferencedMemberVariable(
        {"impresion_especifica"}, {{"class std::basic_ostream<char> &"}},
        "LibroDigital", {"const"}, {"f_expir_"},
        "Revisa qué debe imprimir la definición del método "
        "impresion_especifica en cada clase.");
    ...
    llvm::outs()
        << "No se han encontrado las clases 'Articulo', 'ArticuloAlmacenable', "
           "'Autor', 'Libro', 'Cederron' y/o 'LibroDigital'\n";
  }
  ...
  if (c2.findClass({"Tarjeta"})) {
    ...
    c2.inlineMethod(
        methodNames, parametersMethods, "Tarjeta", {"const", "const", "const"},
        "Sugerencia: incluir marca 'inline' a aquellos métodos con pocas "
        "instrucciones, como 'tipo()', 'numero()' o 'caducidad()'.");
  ...
  if (c3.findClass({"Usuario"})) {
    ...
    c3.numberOfConstructors(
        "Usuario", 1, false,
        "Revisa el enunciado respecto a los constructores en esta clase.");

    c3.friendFunction({"operator<<"}, {{"?"}}, "Usuario",
                      "Revisa si existen funciones que deben ser marcadas como "
                      "amigas de la clase.");
    vector<string> methodNames = {"id", "nombre", "apellidos"};
    vector<vector<string>> parametersMethods = {{}, {}, {}};
    c3.inlineMethod(
        methodNames, parametersMethods, "Usuario", {"const", "const", "const"},
        "Sugerencia: incluir marca 'inline' a aquellos métodos con pocas "
        "instrucciones, como 'id()', 'nombre()' o 'apellidos()'.");
  ...
  if (c4.findClass({"Pedido"})) {
    ...
    c4.invocationsFromHeaders(functionNames, headerName, true,
                              "Revisa de dónde son tomadas las funciones de la "
                              "biblioteca estándar como strlen, strcpy...");

    c4.allPrivateVariableMember("Pedido", "Revisa el acceso a los atributos.");

    c4.numberOfConstructors(
        "Pedido", 1, false,
        "Revisa el enunciado respecto a la construcción de objetos.");

    c4.defaultArgumentsInMethod(
        {"Pedido"}, {{"?"}}, "Pedido", {"?"}, {1}, {{"Fecha()"}},
        "Revisa el enunciado respecto a la construcción de objetos.");

    c4.listInitializerConstructor("Pedido", {"?"},
                                  {"int", "const class Tarjeta *",
                                   "class Fecha", "double"},
                                  "Revisa la lista de inicialización del "
                                  "constructor.");  // Ponemos ? como lista de
                                                    // parámetros porque solo
                                                    // debe haber un constructor.
                                                    // La regla funcionará
                                                    // cualquiera sea el orden en
                                                    // el que se pongan los
                                                    // inicializadores en el
                                                    // constructor

    c4.function({"operator<<"},
                {{"class std::basic_ostream<char> &", "const class Pedido &"}},
                "Revisa el enunciado respecto al operador de extracción.");
  ...
  if (c5.findClass({"Pedido_Articulo", "LineaPedido"})) {
    ...
    c5.numberOfConstructors(
        "LineaPedido", 1, false,
        "Revisa el enunciado respecto a los construcción de objetos.");

    vector<string> params = {"double", "unsigned int"};
    vector<vector<string>> methodsParams;
    methodsParams = {params};

    c5.defaultArgumentsInMethod(
        {"LineaPedido"}, methodsParams, "LineaPedido", {"?"}, {1}, {{"1.*"}},
        "Revisa el enunciado respecto a la construcción de objetos.");

    c5.explicitSpecifiedConstructor(
        "LineaPedido", params,
        "Revisa el enunciado respecto a conversiones implícitas.");

    c5.function(
        {"operator<<"},
        {{"class std::basic_ostream<char> &", "const class LineaPedido &"}},
        "Revisa el lugar de la declaración de los operadores.");

    //'pedir' sobrecargado puede hacerse mediante la busqueda de dos métodos con
    //diferentes parametros.
    c5.method(
        {"pedir", "pedir"},
        {{"class Pedido &", "class Articulo &", "double", "unsigned int"},
         {"class Articulo &", "class Pedido &", "double", "unsigned int"}},
        "Pedido_Articulo", {"noconst", "noconst"},
  ...
  if (c6.findClass({"Usuario_Pedido"})) {
    ...
    c6.invocationsFromHeaders(functionNames, headerName, true,
                              "Revisa de dónde son tomadas las funciones de la "
                              "biblioteca estándar como strlen, strcpy...");
    ...
    c6.notFriendMember("Usuario_Pedido",
                       "Revisa por qué no es necesario incluir friend.");
