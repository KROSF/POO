...
int main() {
  ...
  Usuario lucas("lucas", "Lucas", "Grijánder",
                "Avda. Generalísimo, 654 (Barbate)", "vaeg0Quo"),
      krispin("krispin", "Crispín", "Klánder", "C/ del Jurel, 876 (Barbate)",
              "Pheaj9li"),
  ...
  Tarjeta visa1(Tipo::VISA, Numero("4539 4512 0398 7356"), lucas,
                Fecha(31, 12, 0) + 3 * 365),
      american_express(Tipo::AmericanExpress, Numero("378282246310005"), yoshua,
                       Fecha(30, 11, 0) + 4 * 365),
      dinners_club(Tipo::Mastercard, Numero("30569309025904"), yessi,
                   Fecha(31, 7) + 5 * 365),
      mastercard(Tipo::Mastercard, Numero("555555555555 4444  "), krispin,
                 Fecha(31, 1) + 3 * 365),
      australian_bank_card(Tipo::Maestro, Numero("5610591081018250"), krispin,
                           Fecha(28, 2) + 365),
      jcb(Tipo::VISA, Numero("3530111333300000"), yessi,
          Fecha("31/7/0") + 2 * 365),
      visa2(Tipo::VISA, Numero(" 4222222222222"), lucas, Fecha("28/2/0") + 365);

  // Constructor de Autor: nombre, apellidos, dirección
  Autor Pepe("Pepe", "Leches", "Avda. del Pollo, 55 (Barbate)"),
  ...
  Libro arte_insulto(autores, "100", "El arte del insulto", "1/8/2000", 32.50,
                     245, 5);  // autor = Marchena

  autores.insert(&Carmela);
  Libro montar_bici(autores, "110", "Cómo montar en bici sin sillín",
                    "20/6/1998)", 35.95, 650,
                    100);  // autores = Marchena y Carmela
  autores.clear();

  autores.insert(&Pepe);
  Libro braille(autores, "111", "Aprenda Braille en 5 minutos", "5/10/2002",
                42.10, 150, 300);  // autor = Pepe

  autores.insert(&Carlos);
  Libro baston(autores, "103", "Historia del bastón", "1/2/2001", 39.95, 455,
               4);  // autores = Pepe y Carlos
  ...
  LibroDigital  // autor = Carmela
      eBook2(autores, "035", "Horarios", "20/3/2009)", 9.,
             Fecha("20/6/00") + 365 * 4),
      eBook3(autores, "036", "Exámenes", "1/10/2007", 12.,
             "30/9/2008");  // expdo.
  ...
  Cederron  // autores = Carmela, Marchena y Pepe
      cd3(autores, "220", "Jarabe de Falo", "7/7/2007", 12.90, 547, 30);
  ...
  try {
    ...
    std::cerr << "Error de pedido: tarjeta caducada el " << e.cuando() << endl;
  }
  try {
    Articulo::Autores a;
    Libro anonimo(a, "133", "El Necronomicón", "10/3/2009", 35.00, 210, 4);
  } catch (Articulo::Autores_vacios) {
    std::cerr << "Error de artículo: No se puede crear uno anónimo.\n";
  }
  cout << "\nNúmero total de pedidos: " << Pedido::n_total_pedidos() << '\n'
       << endl;
}
